  ...
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
        ...
        <div className="flex flex-col">
            ...
            <p className="capitalize text-red-400">{exercise.muscles.join(' & ')}</p>
        </div>
            ...
            {['reps', 'rest', 'tempo'].map(info => {
                return (
                    <div key={info} className="flex flex-col p-2 rounded border-[1.5px] border-solid border-slate-900 w-full">
                        ...
                        <p className="font-medium text-[#faa560]">{exercise[info]}</p>
                    </div>
                )
            })}
            <button onClick={incrementSets} className="flex flex-col p-2 rounded border-[1.5px] duration-300 border-solid border-red-900 hover:border-red-600 w-full">
